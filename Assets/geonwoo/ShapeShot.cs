    ...
    void shot()
    {
        ...
        for (int r = 0; r < Vertex; r++)
        {
            for (int i = 1; i <= m; i++)
            {
                ...
                idx2.transform.position = pos.transform.position;

                //정밀한 회전 처리로 모양을 만들어 낸다.
                idx2.transform.rotation = Quaternion.Euler(0, 0, dir - xx[i]);

                //정밀한 속도 처리로 모양을 만들어 낸다.
                idx2.GetComponent<Bullets>().speed = v[i] * Speed / sup;
                #endregion
                ...
                //총알 생성 위치를 (0,0) 좌표로 한다.
                idx3.transform.position = pos.transform.position;

                //정밀한 회전 처리로 모양을 만들어 낸다.
                idx3.transform.rotation = Quaternion.Euler(0, 0, dir);

                //정밀한 속도 처리로 모양을 만들어 낸다.
                idx3.GetComponent<Bullets>().speed = Speed;
                #endregion
